...
router.get("/laden", async (req, res) => {
  try {
    ...
    const [rows] = await db.execute(
      ...
      SELECT
        SUM(CASE
              WHEN naechste_pruefung IS NOT NULL
               AND naechste_pruefung < CURDATE()
              THEN 1 ELSE 0
            END) AS faellig,
        ...
        SUM(CASE
              WHEN naechste_pruefung IS NOT NULL
               AND naechste_pruefung >= CURDATE()
               AND naechste_pruefung <= DATE_ADD(CURDATE(), INTERVAL ? DAY)
              THEN 1 ELSE 0
            END) AS bald_faellig,
      ...
      FROM kunden
      `,
      [days, days] // ✅ WICHTIG: weil 2× '?' im SQL
    );

    res.json(rows[0]);
  } catch (err) {
    console.error("Fehler beim Laden der Service-Stats:", err);
    res.status(500).json({ error: "Fehler beim Laden der Service-Stats" });
  }
});
  ...
  try {
    ...
    const [rows] = await db.execute(
      ...
      WHERE firma   LIKE ?
          ...
          OR telefon LIKE ?
      ORDER BY 
            (firma IS NOT NULL AND firma != '') ASC, nachname ASC, vorname ASC, firma AS
        `,
